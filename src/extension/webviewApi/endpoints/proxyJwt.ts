...
export const proxyJwt = async () => {
  const response = await apiClient.get('/proxy/jwt/')
    .then(response => {
    ...
    .catch(error => {
      ...
      if (error instanceof ApiError) {
        outputChannel.appendLine(`${ error.name } ${ error.response.status }`)

        window.showErrorMessage(`Failed to connect to LLM server. ${ error.response.data.detail || '' }`)
      } else {
